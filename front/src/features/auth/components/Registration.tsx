...
const Registration: FC = () => {
  ...
  const [massageForm, setMassageForm] = React.useState("");
  const dispatch = useAppDispatch();

  const { status, massage, isAuthenticated } = useAppSelector(
    (state) => state.auth
  );
  ...
  const onSubmitHandler = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (password !== re_password) {
      setMassageForm("Passwords do not match");
    } else if (password.length < 6) {
      setMassageForm("Password must be at least 8 characters");
    } else if (
      nameHasError ||
      emailHasError ||
      passwordHasError ||
      ...
      setMassageForm("Password must be less than 128 characters");
    } else if (password.search(/[a-z]/i) < 0) {
      setMassageForm("Password must contain at least one letter.");
    } else if (password.search(/[0-9]/) < 0) {
      setMassageForm("Password must contain at least one digit.");
    } else if (password.search(/[!@#$%^&*]/) < 0) {
      setMassageForm("Password must contain at least one special character.");
    } else {
      ...
      <div className="flex flex-wrap items-center h-screen">
        ...
        <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
          <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
            {status === "error" && <Message massage={massage} />}
            {!(massageForm === "") && <Message massage={massageForm} />}
